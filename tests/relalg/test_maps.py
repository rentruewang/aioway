...
@node_dcls
class SaveLastMapStream(MapExec):
...
@pytest.fixture
def save_last(table_stream: TdictExec):
...
def map_stream(request: pytest.FixtureRequest, save_last: SaveLastMapStream):
    ...
    builder: cabc.Callable[[TdictExec], MapExec] = request.param

    if not callable(builder):
        raise TypeError("The `map_stream` fixture only accepts function parameters.")

    return typing.cast(MapExec, builder(save_last))


def _pred_filter_builder(source):
    return FuncFilterExec(
        source=source,
...
def test_filter(map_stream: TdictExec, save_last: SaveLastMapStream):
...
def _rename_builder(save_last: SaveLastMapStream):
    ...
    return RenameExec(source=save_last, renames=renames)


@pytest.mark.parametrize("map_stream", [_rename_builder], indirect=True)
def test_rename(map_stream: TdictExec, save_last: SaveLastMapStream):
...
def _apply_builder(save_last: SaveLastMapStream):
    ...
    return ApplyExec(source=save_last, apply=func)


@pytest.mark.parametrize("map_stream", [_apply_builder], indirect=True)
def test_apply(map_stream: ApplyExec, save_last: SaveLastMapStream):
...
def _project_builder(save_last: SaveLastMapStream):
    return ProjectExec(source=save_last, subset=["f1d", "i2d"])


@pytest.mark.parametrize("map_stream", [_project_builder], indirect=True)
def test_project(map_stream: TdictExec, save_last: SaveLastMapStream):
...
def test_map_stream_one_to_one(map_stream: MapExec, save_last: SaveLastMapStream):
...
def test_caching(map_stream: TdictExec):
    cached = TdictListExec(list(map_stream))
