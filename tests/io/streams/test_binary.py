...
@pytest.fixture
def lhs_stream(concat_stream: StreamDict) -> CacheStream:
    return CacheStream(concat_stream)


@pytest.fixture
def rhs_stream(joinable_stream: StreamDict) -> CacheStream:
    return CacheStream(joinable_stream)


def test_lhs_stream_length(concat_stream: StreamDict, lhs_stream: StreamDict):
    assert concat_stream.size == lhs_stream.size


def test_rhs_stream_length(joinable_stream: StreamDict, rhs_stream: CacheStream):
...
def test_zip_input_len(
    binary_stream: StreamDict,
    concat_stream: StreamDict,
    rhs_stream: CacheStream,
):
...
def _join_builder(lhs_stream: StreamDict, rhs_stream: CacheStream):
