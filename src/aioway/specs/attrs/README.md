...
There are 2 types of schemas: `Attr` and `dict[str, Attr]` (representing schema in a table), where the latter is a collection of former.

Note:

    `dict[str, Attr]` is not called `Schema`, even though it is a table's `Schema` because:
