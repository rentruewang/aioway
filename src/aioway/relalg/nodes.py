...
class GraphNode[T: GraphNode = typing.Any](abc.ABC):
    ...
    def requires_grad(self) -> bool:
        """
        Whether or not the `torch.Tensor`s generated by this `Exec` `requires_grad`.
    ...
    def is_source(self) -> bool:
        "If the `GraphNode` is not dependent on other `Exec`s, it's considered a source."

        return bool(list(self.deps()))

    def dag(self) -> list[GraphNode]:
        "Order the `Exec`s in their topological order."
    ...
    def replace(
        ...
        or else recursively invoke `.replace(function)` on `Exec` in `.inputs()`.
    ...
    def _repr_abbrev(self) -> str:
        """
        Perform `repr`, but with `Exec` members abbreviated.
        """
